...
const int VALUES[5] = { DOLLAR_VALUE, QUARTER_VALUE, DIME_VALUE, NICKEL_VALUE, PENNY_VALUE };

const int CHANGE_VALUES[8] = { TWENTYDOLLAR_VALUE, TENDOLLAR_VALUE, FIVEDOLLAR_VALUE, DOLLAR_VALUE, QUARTER_VALUE, DIME_VALUE, NICKEL_VALUE, PENNY_VALUE };
...
public:
	...
	string getChange(string input)
	{
		...
		for (int i = 0; i < 8; i++)
		{ amounts[i] = 0; } //set each value to zero.

		parseBestChange(sum, amounts); //parses for the best change

		stringstream ss;
		ss << "This yields: " << amounts[0] << " twenty dollar bill(s), " << amounts[1] << " ten dollar bill(s), " << amounts[2] << " five dollar bill(s), " << amounts[3]  << " one dollar bill(s), " << amounts[4] << " quarter(s), " << amounts[5] << " dime(s), " << amounts[6] << " nickel(s), " << amounts[7] << " penny(ies).";
